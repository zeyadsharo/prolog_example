...
l([]):-!.
l([H|T]):-0 is mod(H,2),write(H),l(T) ; l(T).
...
m([],X,Y):- write('sum of Even: '), write( X), write(' Sum of Odd: '),write(Y),!.
m([H|T],X,Y):-0 is mod(H,2) ,X1 is X+H ,m(T,X1,Y); 1 is mod(H,2), Y1 is Y+H ,m(T,X,Y1);m(T,X1,Y1).
